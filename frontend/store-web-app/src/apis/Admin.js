...
export const getCurrentAdmin = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:8000/admin/protected-route", {
        headers: {
  ...
  export const getOrderProducts = async (orderId = null, productId = null) => {
    ...
    try {
        ...
        console.error('Error fetching order products:', error.response?.data || error.message);
        throw error;
    }
};
  ...
  export const createCategory = async (category) => {
    setAuthToken(); 
    try {
      const response = await apiClient.post(`admin/create-category/?category=${category}`);
      return response.data;
    } catch (error) {
  ...
  export const uploadImages = async (productId, images) => {
    ...
    try {
      ...
      formData.append('main_image_url', images.mainImage); // 'main_image_url' es el nombre del campo esperado en el backend
  ...
  export const updateCategory = async (categoryId, newCategory) => {
    setAuthToken(); 
    try {
      const response = await apiClient.put(`admin/update-category/${categoryId}?new_name=${newCategory}`);
      return response.data;
    } catch (error) {
  ...
  export const updateProduct = async (productId, updateProduct) => {
    setAuthToken(); 
    try {
      const response = await apiClient.put(`admin/update-products-items/${productId}/`, updateProduct);
      return response.data;
    } catch (error) {
      console.error('Error updating category:', error.response?.data || error.message);
      throw error;
  ...
  export const deleteCategory = async (categoryId) => {
    setAuthToken(); 
    try {
      const response = await apiClient.delete(`admin/delete-category/${categoryId}/`);
      return response.data;
    } catch (error) {
  ...
  export const updateUserState = async (userId, newState) => {
    setAuthToken(); 
    try {
      const response = await apiClient.put(`admin/update-state-user/${userId}/?new_state=${newState}`);
      return response.data;
    } catch (error) {
      console.error('Error updating user state:', error.response?.data || error.message);
      throw error;
    }
  };
